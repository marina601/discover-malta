...
class TestTripView(TestCase):
    ...
    def test_trip_details_view(self):
        ...
        response = self.client.get(
            f'/trips/category/{category.slug}/{trip.slug}/')
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_trip_sort_query(self):
        ...
        response = self.client.get('/trips/', {'sort': 'adutt_price',
                                               'direction': 'asc'})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "trips/trips.html")
    ...
    def test_get_category(self):
        ...
        category = Category.objects.create(category_name='Test Category',
                                           slug='test-category')
        response = self.client.get(f'/trips/category/{category.slug}/')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "trips/trips.html")
    ...
    def test_submit_review(self):
        ...
        trip = Trip.objects.create(name='Test Trip', duration=5,
                                   category=category, slug='test-trip',
                                   images='image.jpg')
        user = Account.objects.create(first_name='user', last_name='test',
                                      email='test@test.com',
                                      password='testuser')
        found = resolve(f'/trips/submit_review/{trip.id}/{user.id}/')
        self.assertEqual(found.url_name, "submit_review")
